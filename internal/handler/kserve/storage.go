...
func (kh *Handler) getStoragePvc(ctx context.Context, storageUri, namespace string, nodes GraphNodeMap) {
	...
	// 查找对于的pvc
	pvcObj := nodes.AddNodes(pvcName, namespace, utils.GetCrdKey("pvc"), GraphNodeStatusTrue, nil)
...
func (kh *Handler) getStorageSecret(ctx context.Context, serviceAccountName string, parent *GraphNode, nodes GraphNodeMap) {
	...
	var saObj = nodes.AddNodes(serviceAccountName, "", utils.GetCrdKey("ServiceAccount"), GraphNodeStatusTrue, nil, parent)
	...
	for _, secret := range sa.Secrets {
		ss := &corev1.Secret{}
		var ssObj = nodes.AddNodes(secret.Name, secret.Namespace, utils.GetCrdKey("Secret"), GraphNodeStatusTrue, nil, saObj)
...
func (kh *Handler) getStorage(ctx context.Context, serviceAccountName, namespace string, storageURIs []string, parent *GraphNode, nodes GraphNodeMap) {
	...
	tscObj := nodes.AddNodes(name, "", utils.GetCrdKey(""), status, nil, parent)
